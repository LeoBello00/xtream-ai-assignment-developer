...
def save_model(model, model_name,preferred_preprocessing=0):
    ...
    try:
        ...
        with open(old_model_path + model_name + '.pkl', 'wb') as f:
...
def save_scores(y_test, y_pred, model_name):
    scores = { 'r2_score': r2_score(y_test, y_pred), 'mae': mean_absolute_error(y_test, y_pred) }
    ...
    try:
        ...
        old_scores.to_csv(old_scores_path + model_name + '.csv', index=False)
    except:
        pass
